...
###28.runtime?
1. Objective-c语言的动态特性和面向对象功能就是由运行时系统实现的，运行时系统主要由两部分组成：编译器和运行时系统库，编译器接受Objective-C源代码生成由运行时系统库执行的代码，运行时系统库会与所有的Objective-c程序链接。<br>
2. 运行时系统由下面部分组成：类元素（接口，实现代码，协议，分类，方法，属性，实例变量），对象，对象消息传递(包括动态类型和动态绑定)，动态方法决议，动态加载，对象内审。<br>
3. 运行时系统库是用C语言编写的，在后台起作用，但是它还有公用API，可以直接在代码中使用它。运行时系统系统库API定义了一系列数据类型，函数和常量，大多数都与Objective-C中的语言元素（如对象，类，协议，方法和实例变量等）意义对应。<br>
4. 运行时系统库实现了各种特性和机制，使用它们可以增强应用程序的性能和可扩展性，典型的特性和机制包括方法缓存，虚函数表和dyld共享缓存。

1. 使用Objective-C中的消息传递特性可以调用类和对象中的方法，是一种动态特性，即接收器和接收器中被调用的方法是在运行程序时确定的。<br>
2. 使用动态类型功能可以在运行程序是决定对象的类型，因而能够由运行时因素决定在程序中使用哪种类型的对象，Objective-C使用id数据类型支持动态类型。<br>
3. 动态绑定是指在程序运行时（而不是编译时），将消息与方法对应起来的处理过程。动态绑定功能实现了OOP的多态性，并且能够在不影响已有代码的情况下，将新对象和代码添加到程序中，从而降低对象之间的耦合度。<br>
4. 使用动态方法决议能够以动态方式实现方法。NSObject中含有resolveInstanceMethod：和resloveClassMethod:方法，使用它们能够以动态的方式分别实现由选择器指定的实例和类方法。<br>
5. 使用动态加载功能可以根据需要加载Objective-C程序的可执行代码和源代码，而无需再启动应用程序的时候加载它的所有组件。
Foundation框架中NSObject类中含有非常多用于执行对象内审的方法。<br>
